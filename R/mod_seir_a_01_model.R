...
with(as.list(c(stocks, auxs)), {
    Asymptomatic_Infectious_Period <- max(0.1, sim_state$Incubation_Period_C - 
    ...
    EigenTerm <- sim_state$Total_Infectious_Period_D + sim_state$Total_Infectious_Period_D * 
        sim_state$Proportion_Asymptomatic_f * (-1 + sim_state$Beta_Multiplier_h) - 
        sim_state$Proportion_Asymptomatic_f * (-1 + sim_state$Beta_Multiplier_h) * 
            (sim_state$Incubation_Period_C - sim_state$Latent_Period_L)
    IP01 <- Infectious_Presymptomatic_01/(Asymptomatic_Infectious_Period/2)
    Net_Infectious_Period_for_Infection_Compartments <- max(0.1, 
        sim_state$Total_Infectious_Period_D + sim_state$Latent_Period_L - sim_state$Incubation_Period_C)
    ...
    Beta_Fixed <- sim_state$R0_Input/EigenTerm
    ...
    Beta <- ifelse(sim_state$R0_Fixed_Flag == 0, sim_state$Contact_Multiplier * 
        sim_state$Beta_Calibrated, sim_state$Contact_Multiplier * Beta_Fixed)
    ...
    return(list(c(d_CumulativeCases_dt, d_Exposed_01_dt, d_Exposed_02_dt, 
        ...
        C01_Total_Presymptomatic_Infected = C01_Total_Presymptomatic_Infected, 
        ...
        E01 = E01, E02 = E02, EigenTerm = EigenTerm, IP01 = IP01, 
        Net_Infectious_Period_for_Infection_Compartments = Net_Infectious_Period_for_Infection_Compartments, 
        NQI1 = NQI1, NQI2 = NQI2, ReportedCases = ReportedCases, 
        Total_Exiting_IP02 = Total_Exiting_IP02, ASI1 = ASI1, 
        ASI2 = ASI2, Beta_Fixed = Beta_Fixed, IP02a = IP02a, 
        IP02b = IP02b, Being_Tested = Being_Tested, Beta = Beta, 
        Lambda = Lambda, R0 = R0, IR = IR,  sim_state$Beta_Calibrated, 
         sim_state$Contact_Multiplier, 
         sim_state$Latent_Period_L, 
